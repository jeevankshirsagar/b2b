...
import * as dataFormat from 'components/CRUD/Products/list/ProductsDataFormatters';

<xsl:for-each-group select="./entities[@name='products']/fields[@type='relation_one' or @type='relation_many']" group-by="@ref" >
...
import actions from 'actions/products/productsListActions';
...
{ null && <Button
      ...
      View
        ...
        <Button
          color="info"
          size="xs"
          onClick={() => this.props.dispatch(push(`/admin/products/${cell}/edit`))}
        >
        Edit
      </Button>
      &nbsp;&nbsp;
...
<xsl:for-each select="./entities[@name='products']/fields[@show_in_table = 'yes']">
